...
const wchar_t* Phobos::VersionDescription = L" Phobos testing merge build #" _STR(BUILD_NUMBER) L"+" _STR(MERGE_NUMBER) L"_" _STR(MERGE_PATCH) L". Please test the build before shipping.";
#else
...
#ifndef IS_RELEASE_VER
		if (_stricmp(pArg, "-t.b=" _STR(BUILD_NUMBER) "+" _STR(MERGE_NUMBER) "_" _STR(MERGE_PATCH)) == 0)
		{
			HideWarning = true;
		}
#endif
