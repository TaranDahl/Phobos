...
DEFINE_HOOK(0x51C9B8, InfantryClass_CanFire_HitAndRun1, 0x17)
{
	...
	return ((pThis->CanAttackOnTheMove() && pType && pType->OpportunityFire) || pThis->SpeedPercentage <= 0.1) ? CheckPass : CheckNotPass;
}
...
DEFINE_HOOK(0x51CAAC, InfantryClass_CanFire_HitAndRun2, 0x13)
{
	...
	return ((pThis->CanAttackOnTheMove() && pType && pType->OpportunityFire) || !pThis->Locomotor.GetInterfacePtr()->Is_Moving_Now()) ? CheckPass : CheckNotPass;
}
