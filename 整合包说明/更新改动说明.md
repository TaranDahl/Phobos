...
### 2025.2.??  `Phobos-build-45+3_13` -> `Phobos-build-45+3_14`

 > - `( 0 )` 合并 Phobos-develop 近期所有改动（*2025.2.3 - `commit 32c90bded6f720ad15594d11a32c5f24e11b0fcd`*）
 >   - *`commit d44622f2...`* 暂时降低文档旧式搜索的依赖关系
 >   - *`commit 805da819...`* 新增 `IsAnimDelayedBurst` 选项，可用于让建筑的多段攻击无视蓄力动画
 >   - *`commit 32c90bde...`* 修正说明书的格式错误
 > - `( 3 )` 更改 `AnimDontDelayBurst` 键名为 `IsAnimDelayedBurst` ，且其默认值由 `no` 更改为 `yes` ，该功能现在已被公版合并
 > - `(1/3)` 现在 `BuildingProductionQueue` 会在等待建筑放置时将其余建筑变灰
 ...
 > - `(1/3)` 移除 `Cameo.AuxTechnos` 和 `Cameo.NegTechnos` 选项，新增 `Cameo.RequiredHouses` 和 `Cameo.OverrideTechnos` 选项，详细说明请见《额外功能说明》
