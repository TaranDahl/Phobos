...
### 2025.2.??  `Phobos-build-45+3_17` -> `Phobos-build-45+3_18`
 ...
 > - `( 0 )` 合并 Phobos-develop 近期所有改动（*2025.2.17 - `commit 7e2ed96282f0ee1e2e152b816ebf6a68c1b3769b`*）
 >   - *`commit e4a0f5d1...` -> `commit 7e2ed962...`* 修正说明书的内容或格式错误
 > - `(1/3)` 优化 `Trajectory=Disperse` 功能，其对应的 `Trajectory.Speed` 的上限提高到了 `512` ，同时简化并重写其转向计算方式，现在在大半部分情况下 `Trajectory.Disperse.ROT` 的单位约相当于原本的两倍至三倍，且现在能够进行瞬间回转，现在的单位为每帧最大的旋转角度，默认值从 `30` 降低到 `10`
