...
> **緋紅熱茶**，QQ: 335958461
>> Github: **[绯红热茶/CrimRecya](https://github.com/CrimRecya)** ，当前构建仓库: **[Mix4BEtest](https://github.com/CrimRecya/Phobos/tree/develop-Mix4BEtest)**
...
Trajectory.Engrave.ApplyRangeModifiers=no    ; 布尔值，抛体的 `Trajectory.Engrave.Duration` 的计算（包括为零时的自动计算）是否会受到射程AE的影响
Trajectory.Engrave.AllowFirerTurning=yes     ; 布尔值，抛体在发射后是否允许发射者的朝向大幅度变动
Trajectory.Engrave.Duration=0             ; 整数型，切割过程的持续时间，如果为非正数则会根据 `Trajectory.Engrave.SourceCoord` 和 `Trajectory.Engrave.TargetCoord` 自动计算时间，此时 `Trajectory.Engrave.TargetCoord` 可以视作是切割线段的终点坐标
...
Trajectory.Engrave.ProximityImpact=0         ; 整数型，撞击伤害次数，即抛体在检测到周围有单位时在单位所在的位置【额外】引爆一个弹头，为0代表不启用该逻辑，负数视为无限，最后一次触发是在其位置【额外】引爆一个弹头的同时引爆【本身】（撞击伤害也会对抛体本身的目标造成伤害）
Trajectory.Engrave.ProximityWarhead=         ; Warhead类型，撞击伤害使用的弹头
Trajectory.Engrave.ProximityDamage=0         ; 整数型，撞击伤害的伤害量
Trajectory.Engrave.ProximityRadius=0.7       ; 浮点型，撞击伤害触发的范围，不支持负数（不建议设置太大）
Trajectory.Engrave.ProximityDirect=false     ; 布尔值，撞击伤害是否不引爆弹头而直接对目标造成伤害
Trajectory.Engrave.ProximityMedial=false     ; 布尔值，撞击伤害是否不在目标位置引爆，而是直接在抛体位置引爆
Trajectory.Engrave.ProximityAllies=false     ; 布尔值，撞击伤害是否对盟友生效
Trajectory.Engrave.ProximitySuicide=false    ; 布尔值，撞击次数耗尽抛体是否需要消失
```

```{note}
- 为了视觉效果，最好不要设置让此抛体可被拦截
- 除非必要，否则请给 `Trajectory.Engrave.ProximityRadius` 设置一个比较小的值，过大的值会显著增加性能消耗
```
...
Trajectory.Disperse.LaunchSpeed=0            ; 浮点型，发射设定，抛体【初始】飞行速度
Trajectory.Disperse.Acceleration=10.0        ; 浮点型，通用设定，抛体飞行速度的加速度，按照此值每帧增加或减少速度，巡航速度会逐渐达到 `Trajectory.Speed`
Trajectory.Disperse.ROT=10.0                 ; 浮点型，通用设定，抛体转向速度，即抛体的方向每帧最多能旋转多少角度
...
```{note}
- 关于10个按钮快捷键：
  - 按钮快捷键默认名为 `Fire Super Weapon XX` ，可以在csf中添加名为 `TXT_FIRE_TACTICAL_SW_XX` 和 `TXT_FIRE_TACTICAL_SW_XX_DESC` 的项目来分别重写其在快捷键页面中的选项名称和详细说明（如 “发射超武 %02d” 在游戏中就会根据实际位置自动填写对应数字）
...
```ini
[General]                                    ; 全局通用设置
NoTurret.EarlyTurnToTarget=false             ; 布尔值，所有无炮塔单位在武器冷却时是否会朝向目标

[SOMEUNIT]                                   ; 任意无炮塔车辆
NoTurret.EarlyTurnToTarget=                  ; 布尔值，该无炮塔单位在武器冷却时是否会朝向目标，默认值为 `General` 中 `NoTurret.EarlyTurnToTarget` 的值
```
...
BuildingSell.IgnoreUnsellable=no             ; 布尔值，卖建筑时是否无视建筑的其他所有可能导致无法售卖的条件
...
RateDown.Cover.Value=0                       ; 整数型，单位当前的弹药小于等于 `RateDown.Ammo` 时使用该值替换原本的 `RateDown`
RateDown.Cover.AmmoBelow=-2                  ; 整数型，单位当前的弹药小于等于该值时使用 `RateDown.Cover` 替换原本的 `RateDown`
```
...
Trajectory.Tracing.AllowFirerTurning=yes     ; 布尔值，抛体在发射后是否允许发射者的朝向大幅度变动
...
```ini
[AudioVisual]                                ; 视听通用设置
JumpjetTilt=no                               ; 布尔值，所有悬浮空军是否都会在移动时前倾，转向时侧倾

[SOMEUNIT]                                   ; 任意 `Locomotor=Jumpjet` 的车辆
JumpjetTilt=                                 ; 布尔值，该悬浮空军是否都会在移动时前倾，转向时侧倾，默认值为 `AudioVisual` 中 `JumpjetTiltWhenMoving` 的值
...
ScrollSidebarStripWhenHoldKey=yes            ; 布尔值，是否能够在按下 `Ctrl` 或 `Alt` 或 `Shift` 时使用滚轮滚动侧边栏
```
