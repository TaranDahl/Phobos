...
```{note}
- 在拥有目标的情况下，会每隔索敌时间重新尝试索敌，若存在更高威胁的单位，则会更换目标，若类型相同则不会更换
```
...
[SOMEBUILDING]                               ; 只能是建筑
Cameo.ShouldCount=                           ; 布尔值，是否需要被 `ShowBuildingStatistics` 所统计，默认值为 `BuildLimit` 不为零或不是 `BuildCat=Combat`
```
...
```ini
[General]                                    ; 全局通用设置
ExtendedBuildingPlacing=no                   ; 布尔值，是否允许建筑能在己方单位头顶建造，并尝试散开占据这些格子的单位（只有己方单位和电脑操作的友方单位能够被散开）
