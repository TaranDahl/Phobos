...
- 平台仍在持续更新中，新增内容较多，如果找不到《更新改动说明》中的内容，请善用查找搜索
...
### 1-1. 超武给予超武（ *SW tag that grants 1-time-use super weapons #1199 by FS-21* ）
...
### 1-2. 轰击抛射体增强（ *Trajectory projectiles enhancement #1246 by NaotoYuuki & Ollerus & CrimRecya* ）
...
### 1-3. 移动命令不重置攻击目标（ *Allow vehicles to retain target on move command #1299 by Starkku* ）
...
### 1-4. 触发小队初始载员修复（ *[Simple fix] InitialPayload was invalid in TeamClass #1375 by a851903106* ）

- 仅修复，无ini
- 当触发小队中的载具没有载员时则会尝试填充载具ini内填写的它的初始载员

### 1-5. 自定义坠落伤害（ *Customizing bridge fall damageFalling down damage #1421 by a851903106* ）
...
### 2-41. 无炮塔车辆武器冷却时转向（ 暂无pr ）

- 仅优化，无ini
- 原本对于女娲之类的单位，武器冷却时不会根据目标调整转向，现在它们会一直尝试朝向目标了。
...
Trajectory.Straight.PassDetonateTimer=0      ; 整数型，穿透伤害在抛体发射后多少帧第一次触发，即穿透伤害触发的初始延迟
...
Trajectory.Speed=100.0                       ; 浮点型，弹道抛体的速度，此值定义的是在 [Trajectory.Disperse.UniqueCurve=no] 的情况下时抛体可达到的最大速度（最高速度限制在256）
...
Trajectory.Disperse.WeaponTimer=0            ; 整数型，抛体在发射后多少帧第一次扩散武器，即扩散武器的初始延迟
Trajectory.Disperse.WeaponScope=0            ; 浮点型，抛体扩散武器计时器会在距离目标这么近的距离时才会开始计时，为0代表不启用该逻辑，为负数代表只有在即将引爆本体的瞬间扩散最多一次武器
...
ExpandBuildingPlace=no                       ; 布尔值，是否允许建筑能在己方单位头顶建造，并尝试散开占据这些格子的单位（只有己方单位和电脑操作的友方单位能够被散开），同时允许 `AutoUpgrade` 和 `LimboBuild` 的使用
...
[SOMEBUILDING]                               ; 只能是建筑
AutoUpgrade=no                               ; 布尔值，升级建筑是否不需要手动点击图标放置，而是在完成建造后自动放置
...
```{note}
- 启用 `CheckUnitBaseNormal` 、`ExpandBuildingPlace` 和  `PlacementGrid.Expand`  都会增加一定程度的性能消耗
```
...
```ini
[General]                                    ; 全局通用设置
ExpandAircraftMission=no                     ; 布尔值，开启后会优化 S 的手感， 并能够使用 G 和 Ctrl+Shift ，注意由于飞机类型特殊，这些操作都是一次性不是持续性的，但配合循环路径点可以做出类似区域警戒的效果
```
...
```ini
[AudioVisual]                                ; 视听通用设置
SWSidebarBackground=yes                      ; 布尔值，是否启用超武左侧边栏的背景

[SOMESIDE]                                   ; 任意主阵营
SWSidebarBackground.OnPCX=                   ; PCX文件，需要后缀，玩家属于该阵营时，超武左侧边栏的开关处在未隐藏状态时的按钮素材，素材大小为 10×50，需要 [SWSidebarBackground=yes] 才会绘制，若找不到素材，会显示一个蓝色矩形
SWSidebarBackground.OffPCX=                  ; PCX文件，需要后缀，玩家属于该阵营时，超武左侧边栏的开关处在隐藏状态时的按钮素材，素材大小为 10×50，需要 [SWSidebarBackground=yes] 才会绘制，若找不到素材，会显示一个蓝色矩形
SWSidebarBackground.TopPCX=                  ; PCX文件，需要后缀，玩家属于该阵营时，超武左侧边栏的背景顶部素材，素材大小为 80×20，若找不到素材，会显示一个黑色矩形
SWSidebarBackground.CenterPCX=               ; PCX文件，需要后缀，玩家属于该阵营时，超武左侧边栏的背景中部素材，每一个图标底部对应一个素材，素材大小为 80×50 ，图标左侧距离背景素材边框（同时是屏幕左边缘）5像素，距右侧边框15像素，距上下边框各1像素，若找不到素材，会显示一个黑色矩形
SWSidebarBackground.BottomPCX=               ; PCX文件，需要后缀，玩家属于该阵营时，超武左侧边栏的背景底部素材，素材大小为 80×20，若找不到素材，会显示一个黑色矩形

[SOMESW]                                     ; 任意超武
SW.InScreen.Show=no                          ; 布尔值，该超武是否优先在左侧边栏显示，如果左侧边栏已满（最多显示10个），则溢出的超武图标会被添加回原本的右侧边栏，若之后左侧边栏出现空位，也不再会回到左侧边栏（Ares把这全钩走了），除非重新获得（先失去再获得）该超武的使用权限，故不推荐将所有超武都放在左侧边栏
SW.InScreen.PriorityHouses=                  ; 子阵营列表，若该超武优先在左侧边栏显示，则玩家属于这些子阵营时，会更优先将该超武排在左侧边栏上方（从上到下优先度从高到低）
SW.InScreen.RequiredHouses=                  ; 子阵营列表，若该超武优先在左侧边栏显示，则玩家必须属于这些子阵营才能真正有机会在左侧边栏显示，默认为空，为空代表永远满足此条件
SW.QuickFireAtMouse=no                       ; 布尔值，该超武发射时是否无视除充能和资金外所有条件，强制释放到当前鼠标所在位置，若此时鼠标并未处在战术地图中，则会释放到屏幕中心位置
SW.QuickFireInScreen=no                      ; 布尔值，该超武发射时是否无视除充能和资金外所有条件，强制释放到当前屏幕的中心位置
```
...
```{note}
- 关于10个按钮快捷键：
  - 按钮快捷键默认名为 `Quick Select SW XX` ，可以在csf中添加名为 `TXT_EX_SW_BUTTON_XX` 和 `TXT_EX_SW_BUTTON_XX_DESC` 的项目来分别重写其在快捷键页面中的选项名称和详细说明（XX为数字，数字由小到大分别代表从上到下的按钮顺序）
  - 按钮设置快捷键键位后，当其位置超武在应该显示“就绪”文本时会改为显示其所在位置对应设置的快捷键键位
- 关于1个开关快捷键：
  - 开关快捷键默认名为 `SW sidebar display` ，可以在csf中添加名为 `TXT_EX_SW_SWITCH` 和 `TXT_EX_SW_SWITCH_DESC` 的项目来分别重写其在快捷键页面中的选项名称和详细说明
  - 开关时显示的提示信息，可以在csf中添加名为 `TXT_EX_SW_BAR_VISIBLE` 和 `TXT_EX_SW_BAR_INVISIBLE` 的项目来分别定义开和关时显示的文本
```
...
- 仅优化，无ini
  - 所有单位在执行停止命令时都会同时停止移动攻击行为
  - `Locomotor=Jumpjet` 的单位会在其朝向的格子上停下，不再清除目的地，这应该有助于避免单位陷入袖手旁观的状态

### 3-19. 目标平均分配模式（ *pr #1453* ）
...
Trajectory.Speed=100.0                       ; 弹道抛体的速度，此值定义的是抛体追击单位当前目标的速度（最高速度无限制，负数视为无限）
...
Trajectory.Tracing.WeaponTimer=0             ; 整数型，抛体在首次发射前需要等待多久
...
```ini
[SOMEUNIT]                                   ; 可载员车辆
LeaveTransportKill=no                        ; 布尔值，该单位释放载员时是否不释放而是直接清除全部载员
```
...
### 3-23. AI基地建设逻辑修改（ 暂无pr ）

在 `rulesmd.ini` 之中：

```ini
[AI]                                         ; AI逻辑设置
AINonAutoDeploy=no                           ; 布尔值，电脑在拥有一个建造场后是否不会自动部署基地车
AISetBaseCenter=no                           ; 布尔值，电脑在拥有一个建造场后是否不会设置新部署的基地为新基地中心节点
...
### 3-26. 受到控制停止武器冷却（ 暂无pr ）

在 `rulesmd.ini` 之中：

```ini
[General]                                    ; 全局通用设置
NoRearmInEMPState=no                         ; 布尔值，所有单位处在EMP状态下时，是否不更新武器冷却和弹药装填（不支持飞机弹药）计时器
NoRearmInTemporal=no                         ; 布尔值，所有单位在被超时空武器锁定时，是否不更新武器冷却和弹药装填（不支持飞机弹药）计时器

[SOMETECHNO]                                 ; 任意步兵、车辆、飞机或建筑
NoRearmInEMPState=                           ; 布尔值，当前单位处在EMP状态下时，是否不更新武器冷却和弹药装填（不支持飞机弹药）计时器，默认值为 `General` 中 `NoRearmInEMPState` 的值
NoRearmInTemporal=                           ; 布尔值，当前单位在被超时空武器锁定时，是否不更新武器冷却和弹药装填（不支持飞机弹药）计时器，默认值为 `General` 中 `NoRearmInTemporal` 的值
```

### 3-27. 停火后暂缓盖特冷却计数（ 暂无pr ）
...
RateDown.Ammo=-2                             ; 整数型，单位当前的弹药低于该值时使用 `RateDown.Cover` 替换原本的 `RateDown`
RateDown.Cover=0                             ; 整数型，单位当前的弹药低于 `RateDown.Ammo` 时使用该值替换原本的 `RateDown`
```
...
### 3-30. 避免武器同帧火力溢出（ 暂无pr ）

在 `rulesmd.ini` 之中：

```ini
[SOMEWEAPON]                                 ; 任意武器
NoRepeatFire=0                               ; 整数型，大于0时启用，单位使用该武器向某目标开火后这么多帧内，任何使用具有此标签的武器的单位都不能向其开火
```
...
```ini
[SOMEBUILDING]                               ; 只能是 `LaserFencePost=yes` 的建筑
LaserFencePost.Fence=                        ; Building类型，若此处定义的建筑有 `LaserFence=yes` ，则会以此建筑替换默认使用的激光围墙类型（列表中第一个拥有 `LaserFence=yes` 的建筑类型），且使用不同激光围墙的激光围墙柱之间不会相互连接
```

```{note}
- 如果所有 `LaserFencePost=yes` 的建筑都使用该功能，能够降低性能消耗
```
...
### 3-33. 延迟自动攻击被心控单位（ 暂无pr ）

在 `rulesmd.ini` 之中：

```ini
[General]                                    ; 全局通用设置
AttackMindControlledDelay=0                  ; 整数型，当被心控的单位的被心控时间超过这么多帧时才会被其原本的友军所自动攻击或反击，不影响手动选择攻击，不受无源心控影响
```

### 3-34. 分裂空爆武器支持视觉效果绘制（ 暂无pr ）
