...
[SOMETECHNO]                                 ; 任意步兵、车辆、飞机或建筑
Cameo.AlwaysExist=                           ; 布尔值，该类型是否能够一直显示图标，即在不满足任意建造条件（仅建造条件， 不包括`TechLevel` 、`Owner` & `Cameo.AuxHouses` 、`RequiredHouses` 、`ForbiddenHouses` 、`FactoryOwners` 、`StolenTech` 、`SecretLab` 和 `RequiredTheaters` 等其它限制条件，当满足 `Cameo.AuxTechnos` 时会覆盖前面的 `Owner` & `Cameo.AuxHouses` 条件）时仍会显示灰色图标，且会改变播放EVA语音的时机，默认值为 `General` 中 `Cameo.AlwaysExist` 的值
Cameo.AuxHouses=                             ; 子阵营列表，如果填写，则会与 `Owner` 一起共同对 `Cameo.AlwaysExist` 进行判断
Cameo.AuxTechnos=                            ; Techno列表，如果填写，则当拥有这些类型中任意一种时就会视为 `Owner` & `Cameo.AuxHouses` 的条件通过
