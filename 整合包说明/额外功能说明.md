...
- 提出建议、反馈问题时，请不要在群里直接说，善用@成员或私聊，否则容易被遗忘或者根本没注意到

## 额外功能

### 1. 新的数显类型和显示方式（ *pr #1287* ）
...
- 尽管 `Trajectory.Disperse.Weapons` 中使用的武器支持嵌套使用，但请合理运用
...
- `Trajectory.Parabola.OpenFireMode` 可选类型：
...
```ini
[SOMETECHNO]                                 ; 任意步兵、车辆、飞机或建筑
UniqueTechno=no                              ; 布尔值，该单位是否被视为独特单位
```
