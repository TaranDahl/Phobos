...
### 1-4. 自定义坠落伤害（ *Customizing bridge fall damageFalling down damage #1421 by a851903106* ）
...
```ini
[General]                                    ; 全局通用设置
ExtendedAircraftMissions=no                  ; 布尔值，开启后会优化 S 的手感， 并能够使用 G 和 Ctrl+Shift ，注意由于飞机类型特殊，这些操作都是一次性不是持续性的，但配合循环路径点可以做出类似区域警戒的效果
```
