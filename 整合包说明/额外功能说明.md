...
[SOMEBUILDING]                               ; 只能是建筑
Adjacent.AllowedUnit=                        ; Vehicle列表，该建筑能否被该单位拓展其建筑范围
Adjacent.DisallowedUnit=                     ; Vehicle列表，该建筑能否被该单位拓展其建筑范围
```

```{note}
- `UnitBaseNormal` 和 `UnitBaseForAllyBuilding` 不支持 `Locomotor=Jumpjet` 的单位
